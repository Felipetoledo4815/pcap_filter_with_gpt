    ...
    for pcap_file in os.listdir("pcap_samples"):
        if pcap_file.endswith('.pcap'):
            ...
            if args.exp4:
                ...
                exp4[pcap_file] = []
                for i, group in tqdm(enumerate([query_groups[5]]), total=len(query_groups)):
                    for qn, query in tqdm(enumerate(group[1:]), total=len(group[1:]), leave=False):
                        start = time.time()
                        # Pass text to GPT and get a mysql query
                        mysql_query = get_mysql_query(query)
                        # Send query to MySQL and retrieve result
                        ...
                        result = {
                            "status": status,
                            "group_id": i,
                            "adversarial_type": get_adv_type(qn//5),
                            "group_query": group[0],
                            "modified_query": query,
                            "gt_output": ground_truth[i],
                            "gpt_query": mysql_query,
                ...
                with open('./experiment/exp4_fix.pkl', 'wb') as f:
                    pickle.dump(exp4, f)
